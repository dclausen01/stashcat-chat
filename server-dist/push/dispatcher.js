...
exports.queueMessageEvent = queueMessageEvent;
/**
...
function silentForUser(userId) {
    return (0, mobile_auth_1.listMobileTokensForUser)(userId).then((records) => {
        if (records.length === 0)
            return false;
...
async function flush(userId) {
    ...
    const data = {};
    if (last.deeplink)
        data.deeplink = last.deeplink;
    if (last.msgId)
        data.msgId = last.msgId;
    if (last.channelName)
        data.channelName = last.channelName;
    if (last.senderName)
        data.senderName = last.senderName;
    if (!silent && last.preview)
        data.preview = last.preview;
    if (count > 1)
        data.coalescedCount = String(count);
